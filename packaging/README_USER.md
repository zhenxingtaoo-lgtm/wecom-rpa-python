...
推荐双击 `run-gui.bat` 打开图形界面，选择 `Dry-run 自检` 后点击 `检查环境` 和 `启动运行`。根目录只暴露 bat 是正常的，真正的图形程序在 `app/wecom-rpa-gui.exe`。
...
GUI 运行中会启用 `立即停止` 按钮。点击后程序会请求急停、终止仍在执行的本机 PowerShell 子进程，并立即退出当前 GUI 进程。
